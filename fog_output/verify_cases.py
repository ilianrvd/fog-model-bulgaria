...
EVENT_MIN_HRS = 2       # мин. последователни часове под прага = епизод
...
def fetch_icon_cached(icao, date_str, hour, forecast_hours):
    ...
    if os.path.exists(path):
        ...
        if "cc_series" in prof:
            # JSON връща list-ове; кодът надолу (apply_metar_correction,
            # build_surface_layer) очаква numpy масиви
            for k in ("z", "T", "qv", "p", "u", "v"):
                if k in prof and isinstance(prof[k], list):
                    prof[k] = np.asarray(prof[k], dtype=float)
            for hp in prof.get("hourly_profiles", []):
                for k in ("z", "T", "qv", "u", "v"):
                    if k in hp and isinstance(hp[k], list):
                        hp[k] = np.asarray(hp[k], dtype=float)
            return prof
        print(f"  [кеш] {key}: стар формат (без cc_series) → презаписвам")
        os.remove(path)
...
def evaluate(history, obs_list, hour, date_str):
    ...
    # ── събитийна метрика
    m_eps = _episodes(mod_ev, EVENT_MIN_HRS)
    o_eps = _episodes(obs_ev, EVENT_MIN_HRS)
    if m_eps and o_eps:
        event = "HIT"

...
за избрано летище и за всяка двойка брои засегнатите HIT и FA случаи.
...
def main():
    ...
    ap.add_argument("--icao", default="LBSF")
    args = ap.parse_args()
    ...
    outcome = {r["case_id"]: r["eval"]["event"]
               ...
               and r["case_id"].startswith(args.icao)}
    print(f"[Verify] {os.path.basename(vpath)}   случаи: {len(outcome)}")

    cache = {}
    for p in glob.glob(os.path.join("icon_cache", args.icao + "_*.json")):
        m = re.match(args.icao + r"_(\d{4}-\d{2}-\d{2})_", os.path.basename(p))
        if m:
            cache[m.group(1)] = p
        ...
        for gmin in (8, 10, 12, 14, 16):
            ...
            for cid, (ev, s) in series.items():
                if consecutive_hits(s, vmin, gmin) >= 2:
                    cnt[ev] += 1
                    if ev == "HIT":
                        hit_names.append(cid.replace(args.icao + "_", ""))

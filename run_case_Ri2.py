...
def compute_forecast_changes(hourly_profiles: list, hours: int = 6) -> tuple:
    ...
    от hourly ICON профилите. Запазена за съвместимост (dV_wind се ползва).
    """
...
def compute_Ri_bulk(profile: dict) -> float:
    """
    Bulk Richardson число от ICON профил, изчислено между ~10m и ~200m AGL.

    Ri = (g/θ_mean) * (Δθ/Δz) / (ΔU/Δz)²

    Предимство пред dT850: при топла адвекция НАД студен приземен слой
    инверсията задълбочава → Ri НАРАСТВА → правилно RADIATIVE.
    dT850 четеше промяната на 850 hPa и даваше грешен DYNAMIC сигнал
    (Бургас 18.07.2026, LBGO 01-26).

    Праг: Ri < 1.0 → DYNAMIC; Ri > 1.0 → RADIATIVE
    (0.25 се оказа твърде толерантен на 5-летищния гейт 21.07.2026:
     София CSI 0.33→0.26, FA 19→27 — пускаше RADIATIVE в гранични нощи)
    """
    z_arr = np.asarray(profile.get("z", []), dtype=float)
    T_arr = np.asarray(profile.get("T", []), dtype=float)
    u_arr = np.asarray(profile.get("u", []), dtype=float)
    v_arr = np.asarray(profile.get("v", []), dtype=float)
    p_arr = np.asarray(profile.get("p", []), dtype=float)

    if len(z_arr) < 4 or len(T_arr) < 4:
        return 999.0  # недостатъчно нива → RADIATIVE

    i_bot = int(np.searchsorted(z_arr, 10.0))
    i_top = int(np.searchsorted(z_arr, 200.0))
    i_top = max(i_top, i_bot + 2)
    i_top = min(i_top, len(z_arr) - 1)

    p0_ref = 1e5
    Rd_cp  = 287.05 / 1005.0
    theta_arr = T_arr * (p0_ref / np.maximum(p_arr, 1.0)) ** Rd_cp

    dz = float(z_arr[i_top] - z_arr[i_bot])
    if dz < 1.0:
        return 999.0

    dtheta  = float(theta_arr[i_top] - theta_arr[i_bot])
    theta_m = float(np.mean(theta_arr[i_bot:i_top+1]))
    N2 = (9.81 / theta_m) * (dtheta / dz)

    du = float(u_arr[i_top] - u_arr[i_bot])
    dv = float(v_arr[i_top] - v_arr[i_bot])
    S2 = (du**2 + dv**2) / dz**2 + 1e-6

    Ri = N2 / S2
    return float(np.clip(Ri, -10.0, 50.0))


def diagnose_regime(profile: dict, metar: dict, cfg: dict) -> tuple:
    """
    Определя синоптичния режим чрез bulk Richardson число.

    Заменя dT850+decoupling gate с единно Ri от ICON профила (10-200m):
      Ri < 1.0   → DYNAMIC  (турбулентността не е потисната)
      Ri > 1.0   → RADIATIVE (потисната турбуленция)
      0.25-1.0   → hysteresis (решава reassessment)

    Запазени: V_sfc > 4 m/s (текущ вятър), dV_wind > 5 m/s (засилване),
              крайбрежна адвекция.
    ...
    cur_prof = hourly[0] if hourly else profile
    Ri = compute_Ri_bulk(cur_prof)
    _, dV_wind, _, _ = compute_forecast_changes(hourly, hours=6)

    print(f"[РЕЖИМ] Сигнали: Ri_bulk={Ri:.2f}  V_sfc={V_sfc:.1f}m/s  "
          f"dV={dV_wind:.1f}m/s")
    ...
    # 2. Bulk Ri от граничния слой
    if Ri < 1.0:
        return "dynamic", 3600, f"Ri_bulk={Ri:.2f} < 1.0 → турбулентността не е потисната"

    # 3. Засилване на вятъра напред
    if dV_wind > 5.0:
        return "dynamic", 3600, f"ΔV={dV_wind:.1f}m/s > 5m/s → засилване на вятъра"

    # 4. Крайбрежна адвекция

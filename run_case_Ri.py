...
def compute_Ri_bulk(profile: dict) -> float:
    ...
    Прагове: Ri < 0.25 → DYNAMIC; Ri > 1.0 → RADIATIVE; 0.25-1.0 → hysteresis
    """
...
def diagnose_regime(profile: dict, metar: dict, cfg: dict) -> tuple:
    ...
    Заменя dT850+decoupling gate с единно Ri от ICON профила (10-200m):
      Ri < 0.25  → DYNAMIC  (турбулентно разбъркване)
    ...
    if Ri < 0.25:
        return "dynamic", 3600, f"Ri_bulk={Ri:.2f} < 0.25 → турбулентно разбъркване"
